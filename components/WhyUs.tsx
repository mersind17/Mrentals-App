      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {features.map((feature, idx) => (
            <div
              key={idx}
              className={`group text-center p-10 bg-white/5 border border-white/5 rounded-[48px] hover:bg-white/[0.08] active:bg-white/[0.08] transition-all duration-500 hover:-translate-y-2 active:-translate-y-2 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-16'}`}
              style={{ transitionDelay: `${300 + idx * 150}ms` }}
            >
              <div className="bg-[#acc8a2]/10 text-[#acc8a2] w-24 h-24 rounded-[32px] flex items-center justify-center mx-auto mb-10 group-hover:scale-110 group-active:scale-110 transition-all duration-500 group-hover:bg-[#acc8a2]/20 group-active:bg-[#acc8a2]/20 shadow-inner">
                {feature.icon}
              </div>
